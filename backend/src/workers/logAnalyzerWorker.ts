...
runWorker(
  ...
  async (fields, client) => {
    ...
    const existingLogAnalysis = await prisma.agentAction.findFirst({
      where: { incidentId, agentType: "LOG_ANALYZER" },
      select: { id: true },
    });

    if (existingLogAnalysis) {
      console.log(`[log-analyzer-1] skipping ${incidentId} - log analysis already exists`);
      return;
    }

    if (incident.status !== "OPEN" && incident.status !== "DIAGNOSING") {
